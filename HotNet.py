...
if len(sys.argv) < 8:
	print "Usage: python HotNet.py tested_file mutations_file CNA_file lengths_file BMR delta num_rep"
	...
	print "delta: parameter for heat diffusion"
...
matlab_val_file.write( str(delta)+"\n" )
...
#now call matlab to generate the edges in cc from real data

os.system( "matlab -nodesktop -nosplash -nodisplay -nojvm < code/matlab_for_generateEdges_MR.m")
...
for infile in glob.glob( os.path.join('./tmp/edgesLists/permutation_test/', '*') ):
	if not os.path.isdir( infile) :
		os.remove(infile)
for infile in glob.glob( os.path.join('./tmp/sizes/', '*') ):
	if not os.path.isdir( infile) :
		os.remove(infile)
